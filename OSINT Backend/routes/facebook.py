...
def search_facebook(username):
    """Search Facebook for a public profile by username (no API key required)"""
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        }
        username = username.strip().lstrip('@')
        url = f"https://www.facebook.com/{urllib.parse.quote_plus(username)}"
...
@facebook_bp.route('/search', methods=['GET'])
def facebook_search():
